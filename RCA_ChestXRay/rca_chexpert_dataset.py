...
from deformableNet import DeformableNet, GridSampler


def merge_df(df_img_path, df_lands_path):
    print("Loading image dataset...")
    df_img = pd.read_csv(df_img_path, usecols=["Image"])
    df_img["merge_key"] = df_img["Image"].apply(lambda x: os.path.splitext(x)[0])

    print("Loading landmark dataset...")
    df_lands = pd.read_csv(df_lands_path, usecols=["Path", "Landmark (Mean)"])
    df_lands.rename(columns={"Path": "merge_key", "Landmark (Mean)": "landmarks"}, inplace=True)
    df_lands["merge_key"] = df_lands["merge_key"].apply(lambda x: os.path.splitext(x)[0])# Stripping extension for landmark

    df_merged = pd.merge(df_lands, df_img, on="merge_key", how="left")

    return df_merged


def load_landmarks(path):
    RL_path = "/media/ngaggion/DATA/X-Ray/Chest-xray-landmark-dataset/landmarks/RL/" + path.replace("png", "npy")
    LL_path = "/media/ngaggion/DATA/X-Ray/Chest-xray-landmark-dataset/landmarks/LL/" + path.replace("png", "npy")
    H_path = "/media/ngaggion/DATA/X-Ray/Chest-xray-landmark-dataset/landmarks/H/" + path.replace("png", "npy")
    RL = np.load(RL_path)
    LL = np.load(LL_path)
    H = np.load(H_path)
...
def load_models(config):
    ...
    modelAffine.load_state_dict(torch.load("Training/Affine/affine_pretrained_with_kl/bestMSE.pt"), strict=False)
    modelAffine.eval()

    modelDeformable = DeformableNet((1024, 1024), batchnorm=True).to(config['device'])
    modelDeformable.load_state_dict(torch.load("Training/Deformable/modelDeformable.pt"))
    ...
    modelFinder.load_state_dict(torch.load("bestMSE.pt"), strict=False)
    modelFinder.eval()

    return modelAffine, modelDeformable, modelResampler, modelFinder


def process_images(merged_df, images_train, latent_matrix, modelFinder, modelAffine, modelDeformable,
                   modelResampler, config):
    # Initialize a DataFrame to store the results
    df_reg = pd.DataFrame(columns=['Image', 
                                   'Dice_RCA_Max', 'Dice_RCA_Mean',
                                   'L_Dice_RCA_Max', 'L_Dice_RCA_Mean',
                                   'H_Dice_RCA_Max', 'H_Dice_RCA_Mean'])

    with torch.no_grad():
        count = 0
        img_path = "/media/ngaggion/DATA/X-Ray/BigScale/Datasets/CheXpert/Preprocessed/"
        for _, row in merged_df.iterrows():  # Iterate over merged_df, row will contain each image data
            image = row['Image']
            pred = row['landmarks']
            name = row['merge_key'] + ".jpg"
            
            # Convert the 'Mean' column into landmark coordinates
            landmark_list = [int(x) for x in pred.split(',')]
            landmarks = np.array(landmark_list).reshape(-1, 2)
            RL = landmarks[:44]  
            LL = landmarks[44:94] 
            H = landmarks[94:120] 

            # Generate the mask for the predicted landmarks
            pred_mask = landmark_to_mask(RL, LL, H)

            img = cv2.imread(os.path.join(img_path, image), 0) / 255.0
            ...
            for j in range(0, len(gt_params)):
                ...
                pred_reg = apply_registration_affine(pred_mask, params[0])
            ...
            df_reg.loc[len(df_reg)] = [name, rca_max, rca_avg,
                                        rca_max_lung, rca_avg_lung,
                                        rca_max_heart, rca_avg_heart]

            print('Image:', count, 'of', len(merged_df))
            count += 1
...
def calculate_ground_truth(image_names, config, modelAffine, modelDeformable, source):
    ...
    for img_near in image_names:
        img_target = cv2.imread("/media/ngaggion/DATA/X-Ray/Chest-xray-landmark-dataset/Images/" + img_near, 0) / 255.0
...
if __name__ == '__main__':
    ...
    df_img_path = "/media/ngaggion/DATA/X-Ray/BigScale/Datasets/CheXpert/CheXpert.csv"
    df_lands_path = "/media/ngaggion/DATA/chexmask-database/Preprocessed/CheXpert.csv"
    
    df_merged = merge_df(df_img_path, df_lands_path)
    ...
    df_rca = process_images(df_merged, images_train_heart, latent_matrix, modelFinder,
                            modelAffine, modelDeformable, modelResampler, config)

    # Save the DataFrame to a CSV file
    df_rca.to_csv("/media/ngaggion/DATA/chexmask-database/CHEXPERT_RCA.csv")
