...
def load_models(config):
    ...
    modelReg.load_state_dict(torch.load("Training/Affine/affine_pretrained_with_kl/bestMSE.pt"), strict=False)
    modelReg.eval()
    ...
    for i in range(10):
        ...
        modelPred.load_state_dict(torch.load(f"Training/unet_lh_for_rca/epoch{i}.pt"), strict=False)
        modelPred.eval()
...
if __name__ == '__main__':
    ...
    df_rca = process_images(images_test, images_train_heart, latent_matrix,
                            model_finder, model_reg, model_preds, config)

    # Save the DataFrame to a CSV file
    df_rca.to_csv("df_rca_affine_lh.csv")
