...
from dataset import LandmarksDataset, ToTensor, RandomScaleRotate, AugColor
...
def trainer(target_dataset, source_dataset, val_target_dataset, val_source_dataset, model, config):
    ...
    for epoch in range(config['epochs']):
        ...
        for j in range(0, 180):
            sample_batched = next(iter(target_loader))
            x1, y1 = sample_batched['image'].to(device), sample_batched['landmarks'].to(device)
            
            sample_batched = next(iter(source_loader))
            ...
            alfa = params[:,0] 
            scale = params[:,1:3]
            translate = params[:,3:5]
            
            c = torch.cos(alfa)
            s = torch.sin(alfa)

            # create a rotation matrix for each sample in the batch via broadcasting
            rotation_matrix = torch.stack([c, -s, s, c], dim=1).view(-1, 2, 2)
            
            y2 = y2 - 0.5
            # multiply the rotation matrix with the landmarks to rotate them batchwise
            y2 = torch.bmm(y2, rotation_matrix)            
            y2 = y2 + 0.5
            
            # multiply the scale with the landmarks to scale them batchwise, input shape is B, N, 2, scale each of the N
            # landmarks in each sample of the batch
            y2 = y2 * scale.view(-1, 1, 2)
            y2 = y2 + translate.view(-1, 1, 2)
        
            recloss = F.mse_loss(y1, y2)
        ...
        with torch.no_grad():
            for j in range(0, 170):
                sample_batched = next(iter(val_target_loader))
                x1, y1 = sample_batched['image'].to(device), sample_batched['landmarks'].to(device)
                
                sample_batched = next(iter(val_source_loader))
                x2, y2 = sample_batched['image'].to(device), sample_batched['landmarks'].to(device)
                ...
                alfa = params[0,0]
                scale = params[0,1:3]
                translate = params[0,3:5]
                
                rotation_matrix = torch.tensor([[torch.cos(alfa), -torch.sin(alfa)], [torch.sin(alfa), torch.cos(alfa)]]).to(device)

                y2 = y2 - 0.5
                y2 = torch.matmul(y2, rotation_matrix)
                y2 = y2 + 0.5
