...
def merge_df(df_img_path, df_lands_path):
    ...
    df_img["merge_key"] = df_img["Image"].apply(lambda x: Path(x).stem)  # Stripping extension for image

    print("Loading landmark dataset...")
    df_lands = pd.read_csv(df_lands_path, usecols=["ImageID", "Mean"])
    df_lands.rename(columns={"ImageID": "merge_key", "Mean": "landmarks"}, inplace=True)
    df_lands["merge_key"] = df_lands["merge_key"].apply(lambda x: Path(x).stem)  # Stripping extension for landmark
...
if __name__ == '__main__':
    ...
    df_img_path = "/home/ngaggion/DATA/X-Ray/BigScale/Datasets/Padchest/Padchest_clean_RCA.csv"
    df_lands_path = "/media/ngaggion/DATA/chexmask-database/Preprocessed/Padchest.csv"
    
    #df_merged = merge_df(df_img_path, df_lands_path)
    df_lands = pd.read_csv(df_lands_path, usecols=['Image', 'Landmark (Mean)', 'ImageID'])
    ...
    print('Computing RCA')
    df_rca = process_images(df_lands, images_train_heart, latent_matrix, modelFinder,
                            modelAffine, modelDeformable, modelResampler, config)

    print('Saving to file')
    # Save the DataFrame to a CSV file
    df_rca.to_csv("/media/ngaggion/DATA/chexmask-database/Padchest_RCA.csv")